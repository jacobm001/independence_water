...
% The paper headers
\markboth{Oregon State Univeristy,~CS~462, Group~31, Abzu, April~2016}%
...
\lstset{language=Java,caption={The drawDayChart function demonstrates how routes are used in the front end to return a csv formatted string. This string is parsed to create visualizations.},label=drawDayChart}
\begin{lstlisting}
...
\lstset{language=SQL,caption={The database schema prevents duplicated entries from being added to the database},label=SQL_meter_read}
\begin{lstlisting}
...
\lstset{language=PHP,caption={This figure shows function that converts the Master Meter historical export to the database.},label=DataImporter::submit_file()}
\begin{lstlisting}
